...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--genotype", help="genotype matrix for each donor and locus", required=True)
    parser.add_argument("-i", "--index", help="index of variants selected for clustering", required=True)
    parser.add_argument("-v", "--vcf", help="vcf file used for clustering", required=True)
    parser.add_argument("-r", "--ref_vcf", help="variants for each donor in multisample vcf file", required=True)
    ...
    with subprocess.Popen(["bcftools", "view", args.ref_vcf], stdout=subprocess.PIPE) as proc:
        for row in proc.stdout:
            ...
            if len(donors) != genotypes.shape[0]:
                raise ValueError(f"Number of donors given in genotype matrix {genotypes.shape[0]} does not match the number of donors given in reference vcf file {len(donors)}.")
            break

    # Make true variants compatible with genotype matrix
    distances = np.zeros((len(donors), len(genotypes)))
    variant_idx = 0
    ...
    print("# Count matrix")
    np.savetxt(sys.stdout, distances)
